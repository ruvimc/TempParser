...
type
  ...
  public
    ...
    property UnitLinesCount: integer read FUnitLinesCount;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        // uses
        if (not LParsingStarted) and (Pos(LFileData[i].Trim.ToLower, 'uses') > 0)
        then
...
begin
  ...
  UnitParser.ParseFile(FUnitPath, FUsesList, FUnitLinesCount );
  ShowMessage(inttostr(FUnitLinesCount));
